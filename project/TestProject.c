...
void USART_Init()
{
    TRISC.B6 = 1;
    TRISC.B7 = 1;
    SPBRG=16;  //baud rate=9600, SPBRG = 10M/64*(9600+1)
    TXSTA=0b00100000;
    RCSTA=0b10010000;
}
...
int counter_2 = 0;
void main(){
     // Осцилятор работает на частоте 8 МГц
      IRCF2_bit = 1;
      IRCF1_bit = 1;
      IRCF0_bit = 1;

      lcd_Init(); // Инициализация дисплея
      TRISB = 0b00001111; // выводы 4-7 порта В настроены на вывод, 0-3 на вход
      ...
      // Программа вначале находится в состоянии ожидания 1 числа
      state = WAIT_FIRST_DIGIT;
      // Настройка таймера
      TMR0L = 0xe0;
      TMR0H = 0xb1;
      ...
      while(1)
      {       
             currentKeyState = answer_key_state();  // Чтение состояния клавиатуры
             
             if((currentKeyState.b4 != backKeyState.b4) || (currentKeyState.b5 != backKeyState.b5) 
             || (currentKeyState.b6 != backKeyState.b6) || (currentKeyState.b7 != backKeyState.b7)){
                   T0CON.TMR0ON = 0;
                   TMR0L = 0xe0;
                   TMR0H = 0xb1;
                   INTCON.TMR0IF = 0;
              ...
              backKeyState = currentKeyState; //Запоминаем старое состояние клавиатуры
              if(INTCON.TMR0IF){
                  T0CON.TMR0ON = 0;
                  INTCON.TMR0IF = 0;
                  buf = read_key_state(currentKeyState);
                  if (buf != 'N') { // Чтение ввода с мини-клавиатуры
                      decodPressedKey(buf);
                  };
              }
              if (PIR1.RCIF) { // Чтение из USART
                 buf = USART_RxChar();
                 decodPressedKey(buf);
              }
              if( buf == 'C'){ // Очищение дисплея
                 lcd_Command(0b00000001);
                 clear_all();
              }
